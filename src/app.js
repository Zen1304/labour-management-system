...
async function renderPayrollList(user, query) {
  ...
  <div class="table-wrap"><table>
    <tr><th>Period</th><th>Status</th><th>Generated by</th><th>Generated at</th><th>Net total</th><th></th><th></th><th></th></tr>
    ${runs
      .map(
        ...
        <td>${
          canGenerate && (r.flagged || r.status === 'pending_verification')
            ? `<form class="inline" method="POST" action="/payroll/${r.id}/delete" onsubmit="return confirm('Permanently delete this payroll run and all of its items, deductions, and site data? This CANNOT be undone.')"><button class="btn danger small" type="submit">Delete</button></form>`
            : ''
        }</td>
      </tr>`
      )
      .join('')}
    ${runs.length === 0 ? '<tr><td colspan="8" class="muted">No payroll runs yet.</td></tr>' : ''}
  </table></div>
...
async function renderPayrollDetail(runId, user) {
  ...
  <div class="card muted" style="margin-bottom:20px">
    ...
    ${
      canGenerate && (run.flagged || run.status === 'pending_verification')
        ? `<form class="inline" method="POST" action="/payroll/${run.id}/delete" style="margin-left:10px" onsubmit="return confirm('Permanently delete this payroll run and all of its items, deductions, and site data? This CANNOT be undone.')"><button class="btn danger small" type="submit">Delete permanently</button></form>`
        : ''
    }
  </div>
...
async function handleRequest(req, res) {
  ...
  // Permanent, irreversible removal of a generated payroll run and every row
  // that hangs off it — payroll_items, payroll_deductions,
  // payroll_item_sites, payroll_run_site_verifications. Unlike Flag (which
  // just frees up the wage period while keeping the record for history),
  // this actually erases the data, so it's deliberately guarded harder: only
  // a Pending-verification or already-Flagged run can be hard-deleted — a
  // Verified/Completed run has been signed off and likely paid out, and
  // must be flagged first (Zen's explicit call) before it can be deleted.
  const payrollDeleteMatch = pathname.match(/^\/payroll\/(\d+)\/delete$/);
  if (payrollDeleteMatch && req.method === 'POST') {
    if (!PAYROLL_GENERATE_ROLES.includes(user.role)) return forbidden(res, user, pathname, layout);
    const runId = payrollDeleteMatch[1];
    const run = (await pool.query('SELECT * FROM payroll_runs WHERE id = $1', [runId])).rows[0];
    if (!run) return send(res, 404, 'Not found');
    if (!run.flagged && run.status !== 'pending_verification') {
      return send(
        res,
        400,
        layout({
          ...
          flash: {
            type: 'error',
            message: 'This run is Verified or Completed and can\'t be permanently deleted directly — flag it first if it truly needs to be removed.',
          },
          body: await renderPayrollDetail(runId, user),
        })
      );
    }
    // Same single-connection-transaction pattern as the user hard-delete
    // above — every statement (including the audit snapshot) must commit
    // or roll back together.
    const deleteClient = await pool.connect();
    try {
      await deleteClient.query('BEGIN');
      const itemIds = (await deleteClient.query('SELECT id FROM payroll_items WHERE payroll_run_id = $1', [runId])).rows.map((r) => r.id);
      // Snapshot BEFORE deleting, same as the user hard-delete above — this
      // audit row only persists if the whole transaction commits.
      await deleteClient.query(
        ...
        [
          ...
          'payroll_run',
          runId,
          `${run.period_start} → ${run.period_end} (status: ${run.status}${run.flagged ? ', flagged' : ''}, ${itemIds.length} item(s)) — permanently deleted`,
          nowSqliteStyle(),
        ]
      );
      if (itemIds.length) {
        await deleteClient.query('DELETE FROM payroll_deductions WHERE payroll_item_id = ANY($1::int[])', [itemIds]);
        await deleteClient.query('DELETE FROM payroll_item_sites WHERE payroll_item_id = ANY($1::int[])', [itemIds]);
      }
      await deleteClient.query('DELETE FROM payroll_run_site_verifications WHERE payroll_run_id = $1', [runId]);
      await deleteClient.query('DELETE FROM payroll_items WHERE payroll_run_id = $1', [runId]);
      await deleteClient.query('DELETE FROM payroll_runs WHERE id = $1', [runId]);
